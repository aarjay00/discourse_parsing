...
def updateSenseIndex(index, conn , sense) :
    ...
    if conn not in index.iterkeys() :
            index[conn] = {}
            index[conn][sense] = 1
    else :
            if sense not in index[conn].iterkeys() :
                    index[conn][sense] = 1
            else :
                    index[conn][sense] += 1
    return index
