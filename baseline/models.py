...
def runModel(featureCollection, featureDescCollection, classList, analysisFolderName, cycleLen, yesWt=1, noWt=1):
    ...
    for iteration in range(0, cycleLen):
        ...
        for classLabel in classList:
            try:
                precision[classLabel] = (1.0 * truePositives[classLabel]) / (
                    truePositives[classLabel] + falsePositives[classLabel])
                avgPrecision[classLabel] = (avgPrecision[classLabel] * iteration + precision[classLabel]) / (
                    iteration + 1)
            except:
                print "e1"
            try:
    ...
    for classLabel in classList:
        FD.write(classLabel + "\n")
        FD.write(str(avgPrecision[classLabel]) + " " + str(avgRecall[classLabel]) + " " + str(
            avgModelScore[classLabel]) + " " + str((2 * avgPrecision[classLabel] * avgRecall[classLabel]) / (
            avgRecall[classLabel] + avgPrecision[classLabel])) + "\n")
