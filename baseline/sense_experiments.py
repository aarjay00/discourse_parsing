...
def genFeatureSingleConn(conn, label, discourseFile):
    ...
    feature = Feature("lists/compConnectiveList.list", "lists/tagSet.list", "lists/chunkSet.list", discourseFile,
                      discourseFile.globalWordList, discourseFile.sentenceList, conn)
    ...
    feature.tagNeighbor(conn, 2)
    feature.wordNeighbor(conn,1)
    feature.wordNeighbor(conn,-1)
    feature.wordNeighbor(conn,2)
    feature.wordNeighbor(conn,-2)
    feature.wordNeighbor(conn,3)
    feature.wordNeighbor(conn,-3)
...
def genFeatureSplitConn(conn, label, discourseFile):
    ...
    for p in conn[1]:
        print discourseFile.globalWordList[p].word,
    feature = Feature("lists/splitConnectiveList.list", "lists/tagSet.list", "lists/chunkSet.list", discourseFile,
                      discourseFile.globalWordList, discourseFile.sentenceList, conn)
...
for discourseFile in discourseFileCollection:
    ...
    for conn in singleSet:
        chunkSeqLen1 = (chunkSeqLen1 * i + 1.0 * len(
            getChunkSeq(wordList[conn[0]].arg1Span, wordList, sentenceList))) / (i + 1)
        chunkSeqLen2 = (chunkSeqLen2 * i + 1.0 * len(
            getChunkSeq(wordList[conn[0]].arg2Span, wordList, sentenceList))) / (i + 1)
        i += 1
        ...
        featureCollectionSingle.append(
            genFeatureSingleConn(conn, (wordList[conn[0]].sense).split(".")[0], discourseFile))
        featureDescInst = featureDesc(discourseFile.rawFileName, wordList[conn[0]].sentenceNum,
                                      "Single Connective Sense Identification", wordList[conn[0]].sense,
                                      len(featureDescSingleCollection))
...
createDirectory("processedData/senseFeatureCollection/")
for idx,featureSingleConn in enumerate(featureCollectionSingle):
    exportModel("processedData/senseFeatureCollection/{}".format(idx),featureSingleConn)

createDirectory("processedData/senseFeatureDescriptionCollection/")
for idx,featureDescSingleConn in enumerate(featureDescSingleCollection):
    exportModel("processedData/senseFeatureDescriptionCollection/{}".format(idx),featureDescSingleConn)
...
for iteration in range(0, time):
    ...
    x, y, z, err, corr, extra = runModel(featureCollectionSingle, featureDescSingleCollection, classList,
                                         "sense_identification", 15, 1, 1)
    for key in x.keys():
