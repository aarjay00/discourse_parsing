...
class Feature():
    def __init__(self, word_dictionary_path, tag_path, chunk_path, discourse_file, global_word_list, sentence_list,
                 conn=None):
    ...
    def wordNeighbor(self, wordList, offSet):
        ...
        if (offSet < 0):
            if (wordList[0] + offSet < 0):
                words.append("First")
                self.featureList.append(("wordNeighbor_" + str(offSet), "First"))

            else:
                words.append(self.globalWordList[wordList[0] + offSet].word)
                self.featureList.append(("wordNeighbor_" + str(offSet), self.globalWordList[wordList[0] + offSet].word))
        else:
            if (wordList[-1] + offSet >= len(self.globalWordList)):
                words.append("Last")
                self.featureList.append(("wordNeighbor_" + str(offSet), "Last"))
            else:
                words.append(self.globalWordList[wordList[-1] + offSet].word)
                self.featureList.append(("wordNeighbor_" + str(offSet), self.globalWordList[wordList[-1] + offSet].word))
    ...
    def tagNeighbor(self, wordList, offSet):
        ...
        tagList = []
        if (offSet < 0):
            if (wordList[0] + offSet < 0 or self.globalWordList[wordList[0] + offSet].sentenceNum !=
                self.globalWordList[wordList[0]].sentenceNum):
                #		if(wordList[0]+offSet<0):
                tagList.append("First")
            else:
                tagList.append(self.globalWordList[wordList[0] + offSet].wordTag)
        else:
            if (wordList[-1] + offSet >= len(self.globalWordList) or self.globalWordList[
                    wordList[-1] + offSet].sentenceNum != self.globalWordList[wordList[-1] + offSet].sentenceNum):
    ...
    def tagCombo(self, wordList, offSet1, offSet2):
        ...
        if (offSet1 == 0):
            ...
            if (wordList[0] + offSet1 < 0 or self.globalWordList[wordList[0] + offSet1].sentenceNum !=
                self.globalWordList[wordList[0]].sentenceNum):
                tag = "First"
            else:
                tag = self.globalWordList[wordList[0] + offSet1].wordTag
        else:
            if (wordList[-1] + offSet1 >= len(self.globalWordList) or self.globalWordList[wordList[-1]].sentenceNum !=
                self.globalWordList[wordList[-1]].sentenceNum):
                tag = "Last"
            else:
        ...
        if (offSet2 == 0):
            ...
            if (wordList[0] + offSet2 < 0 or self.globalWordList[wordList[0] + offSet2].sentenceNum !=
                self.globalWordList[wordList[0]].sentenceNum):
                tag = tag + " First"
            else:
                tag = tag + " " + self.globalWordList[wordList[0] + offSet2].wordTag
        else:
            if (wordList[-1] + offSet2 >= len(self.globalWordList) or self.globalWordList[wordList[-1]].sentenceNum !=
                self.globalWordList[wordList[-1]].sentenceNum):
                tag = tag + " Last"
            else:
    ...
    def hasNodeRelationSpecific(self, conn, connective, nodeRelationList, node, nodeDict, maxLevel):
        self.description = self.description + " hasNodeRelationSpecific-" + getSpan(conn,
                                                                                    self.globalWordList) + "-" + str(
            nodeRelationList)
    ...
    def tathaFeature2(self, conn, nodeName, nodeDict, c, a, b):
        ...
        if (self.globalWordList[conn[0]].word != u'\u0924\u0925\u093e'):
            ...
            return [0]
        qc = False
        if ((self.globalWordList[conn[0] - 1].wordTag == "QC" or self.globalWordList[conn[0] - 2].wordTag == "QC" or
                     self.globalWordList[conn[0] - 3].wordTag == "QC") and (
                            self.globalWordList[conn[0] + 1].wordTag == "QC" or self.globalWordList[
                            conn[0] + 2].wordTag == "QC" or self.globalWordList[conn[0] + 3].wordTag == "QC")):
            qc = True
        childList = nodeDict[nodeName].childList
        childVGF = 0
        for child in nodeDict[nodeName].childList:
    ...
    def isseFeature(self, conn, nodeName, nodeDict, c, a, b):
        ...
        if (getSpan(conn, self.globalWordList) != u'\u0907\u0938\u0938\u0947'):
            ...
            return
        if (hasChild(nodeName, nodeDict, "CCP") or node.getChunkName(node.nodeParent) == "CCP" or (
                        node.nodeParent != "None" and node.getChunkName(
                    nodeDict[node.nodeParent].nodeParent) == "CCP")):
            if (self.globalWordList[conn[
                0] - 1].word == u'\u0915\u094d\u092f\u094b\u0902\u0915\u093f' or node.nodeRelation != "rh"):
                self.featureList.append(("isseFeature", "No"))
    ...
    def lekinFeature2(self, conn, nodeName, nodeDict, c, a, b):
        ...
        while (node.nodeParent != "None"):
            ...
            for wordNum in self.sentenceList[self.globalWordList[conn[0]].sentenceNum].chunkList[
                nodeParent.chunkNum].wordNumList:
                print "\thalanki", self.globalWordList[wordNum].word
                if (self.globalWordList[wordNum].word == u'\u0939\u093e\u0932\u093e\u0902\u0915\u093f' or
                            self.globalWordList[wordNum].word == u'\u0939\u093e\u0932\u093e\u0901\u0915\u093f'):
                    halankiParent = True
    ...
    def parFeature(self, conn):
        ...
        if (getSpan(conn, self.globalWordList) != u'\u092a\u0930'):
            ...
            return [0]
        print "got par"
        checkList = [u'\u0916\u093e\u0938\u0924\u094c\u0930', u'\u0935\u0939\u0940\u0902', u'\u0909\u0938',
                     u'\u0907\u0938', u'\u0907\u0938 \u092c\u093e\u0924']
    ...
    def toRootFeature(self, conn, node, nodeDict):
        ...
        sentencePrevSpan = getSpan(sentencePrev, self.globalWordList)
        print sentencePrevSpan
        checkList = [u'\u091c\u0948\u0938\u0947 \u0939\u0940', u'\u092f\u0926\u093f', u'\u0905\u0917\u0930',
                     u'\u091c\u092c', u'\u0935\u0939\u0940\u0902']
        for item in checkList:
            if item in sentencePrevSpan:
                self.featureList.append(("toRootFeature", 0))
    ...
    def keliyeFeature(self, conn, node, nodeDict, label):
        ...
        if (getSpan(conn, self.globalWordList) != u'\u0915\u0947 \u0932\u093f\u090f'):
            ...
            self.featureVector.append(0)
            return 0
        print "ke liye", label, hasChildRelation(node.nodeName, nodeDict, "k3"), hasChildRelation(node.nodeName,
                                                                                                  nodeDict,
                                                                                                  "k7t"), hasChildRelation(
            node.nodeName, nodeDict, "k1")
        if (hasChildRelation(node.nodeName, nodeDict, "k3") or hasChildRelation(node.nodeName, nodeDict,
                                                                                "k7t") or hasChildRelation(
            node.nodeName, nodeDict, "k1")):
    ...
    def halankiFeature(self, conn):
        ...
        pos = conn[0] + 1
        print "halanki", pos
        while (pos < len(self.globalWordList) and self.globalWordList[pos].sentenceNum == self.globalWordList[
            conn[0]].sentenceNum):
